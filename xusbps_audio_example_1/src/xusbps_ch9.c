...
#pragma data_alignment = 32
...
#ifdef CH9_DEBUG
		printf("XUSBPS_REQ_GET_STATUS %d\r\n", SetupData->bmRequestType);
#endif
	...
	case XUSBPS_REQ_GET_INTERFACE:		//10
#ifdef CH9_DEBUG
...
#ifdef CH9_DEBUG
			printf("DEVICE_DESC or QUALIFIER->ReplyLen: %d\r\n", ReplyLen);
			for(cnt = 0; cnt < ReplyLen;cnt++){
				printf("%d: 0x%x\r\n", cnt,Reply[cnt]);
			}
#endif
...
#ifdef CH9_DEBUG
			printf("GET_CONFIGURATION->ReplyLen: %d\r\n", ReplyLen);
			for(cnt = 0; cnt < ReplyLen;cnt++){
				printf("%d: 0x%x\r\n", cnt,Reply[cnt]);
			}
#endif
	...
	case XUSBPS_REQ_SET_INTERFACE:		//11

#ifdef CH9_DEBUG
		printf("set interface %d/%d\r\n", SetupData->wValue, SetupData->wIndex);	//1,1
#endif
		/* Only ISO supported */
		if(InstancePtr->AppData !=NULL)
