...
#define ASYNC_MODE				//comment out:adaptive,enable:ASYNC
...
extern s32 Index;
extern s32 Residue;
extern s32 Residue_In;
extern u8 FirstPktFrame;
extern u8 FirstPktFrame_In;
...
USB_CONFIG __attribute__ ((aligned(16))) config2 = {
	{/*
	  * Std Config
	  */
		sizeof(USB_STD_CFG_DESC),	/* bLength :9*/
		XUSBPS_TYPE_CONFIG_DESC,	/* bDescriptorType :2*/
		sizeof(USB_CONFIG),		/* wTotalLength */
#ifdef USB_IN_ENABLE
		0x03,				/* No. Of interfaces 3 IN有効時はインタフェース3個*/
#else
		0x02,				/* No. Of interfaces 2 IN無効時はインタフェース2個*/
#endif
...
#ifdef USB_IN_ENABLE
		0x03,					/* bInterfaceCount 3 IN有効時はインタフェース3個*/	//このアソシエーションに関連付けられているインターフェースの数
#else
		0x02,					/* bInterfaceCount 2 IN無効時はインタフェース2個*/
#endif
	...
	{/*
	  ...
	  */
		...
		XUSBPS_DT_CS_INTERFACE,		/* bDescriptorType */
		0x01,				/* bDescriptorSubtype */			//UAC_MS_HEADER or UAC_HEADER:0x01,
		0x00,				/* bcdADC - Audio Class 1.0 L */
		...
		(sizeof(UAC2_AC_HEADER_DESC) +
		 ...
		 (sizeof(UAC2_INPUT_TERMINAL_DESC) +
		  ...
		  sizeof(UAC2_OUTPUT_TERMINAL_DESC))
#ifdef USB_IN_ENABLE
		  * 2
#endif
		  ),	/* wTotalLength */
	...
	{/*
	  ...
	  */
		...
		XUSBPS_DT_CS_INTERFACE,		/* bDescriptorType */
		UAC2_CLOCK_SOURCE,		/* bDescriptorSubtype 0x0A*/
		USB_CLK_SRC_ID,			/* bClockId */
		UAC2_CLOCK_SOURCE_TYPE_INT_FIXED,	/* bmAttributes */
		(0x3 << CLK_FREQ_CTRL),	/* bmControls */
		0x00,					/* bAssocTerminal */
		0x06					/* iClockSource */
	},
	{/*
	...
	{/*
	  ...
	  */
		...
		AUDIO_CHANNEL_NUM,		/* bNrChannels */	//物理チャネル数:8		//AUDIO_CHANNEL_NUM
	...
	{/*
	  ...
	  */
		...
		XUSBPS_DT_CS_INTERFACE,	/* bDescriptorType */
		UAC_FEATURE_UNIT,		/* bDescriptorSubtype */
		OUT_FETR_UNT_ID,		/* bUnitID *///7
		USB_OUT_IT_ID,			/* bSourceID *///1
		{
...
//			#define UAC_INPUT_TERMINAL_PROC_MICROPHONE_ARRAY	0x206
		...
		0x01,				/* wTerminalType L */		// UAC_INPUT_TERMINAL_MICROPHONE
		0x02,				/* wTerminalType H */
		0x00,				/* bAssocTerminal */
		USB_CLK_SEL_ID,			/* bCSourceId */
		AUDIO_CHANNEL_NUM,		/* bNrChannels */
	...
	{/*
	  ...
	  */
		...
		0x01,				/* bAlternateSetting */
#ifdef ASYNC_MODE
		0x02,				/* bNumEndpoints */		//フィードバックエンドポイントも足す
#else
		0x01,				/* bNumEndpoints */
#endif
		XUSBPS_CLASS_AUDIO,		/* bInterfaceClass */
	...
	{/*
	  * STD OUT Endpoint		//PC -> ZYNQ
	  */
		sizeof(USB_EP_DESC),	/* bLength */
		XUSBPS_TYPE_ENDPOINT_CFG_DESC,		/* bType */
		USB_EP1_OUT,			/* bEndpoint OUT endpoint address 0 */
#ifdef ASYNC_MODE
		0x01 | USB_ENDPOINT_SYNC_ASYNC,	/* Isochronous, adaptive *///0x05
#else
		0x01 | USB_ENDPOINT_SYNC_ADAPTIVE,	/* Isochronous, adaptive *///0x09	//D5~4:0x00-データ転送のためのエンドポイント
																		//D5~4:0x10-暗黙のフィードバックを行うエンドポイント
#endif
		0x00,				/* bMaxPacketSizeL */
		0x04,				/* bMaxPacketSizeH */	//1024byte
		AUDIO_INTERVAL			/* bInterval */		//ポーリング周期（2^(bInterval-1)）
	},
	{/*
	  ...
	  */
		...
		XUSBPS_DT_CS_ENDPOINT,		/* bDescriptorType */
		UAC_EP_GENERAL,			/* bDescriptorSubtype */
		0x00,				/* bmAttributes *///0x80:MaxPacketsOnly(normally:0x00)
		0x00,				/* bmControls *///D5-4:under run controll ,D3-2:over run controll ,D1-0:pitchcontroll (P138参照)
		0x00,				/* bLockDelayUnits */
		0x00,				/* wLockDelayL */
		0x00				/* wLockDelayH */
	},
	{/*
	  * STD AS ISO Feedback OUT Endpoint
	  */
		sizeof(UAC2_ISO_FB_EP_DESC),		/* bLength */
		XUSBPS_TYPE_ENDPOINT_CFG_DESC,	/* bType */	//0x05
		0x81,			/* bEndpoint OUT endpoint address 0 */
		0x11,			/* bmAttributes */
		0x00,				/* bMaxPacketSizeL */
		0x04,				/* bMaxPacketSizeH */
		AUDIO_INTERVAL			/* bInterval */		//0x04
	},
	...
	{/*
			  * Audio Streaming OUT Interface - Alt2		//fordepth
			  * P223-224,P123
			  */
				sizeof(USB_STD_IF_DESC),	/* bLength */
				XUSBPS_TYPE_IF_CFG_DESC,	/* bDescriptorType */
		...
		{/*
		  ...
		  */
			...
			0x00,  							/* bmFormatsL4 */
			AUDIO_CHANNEL_NUM,				/* bNrChannels */
			0x3F,							/* bmChannelConfigL1 */	//FRC,FLC,BR~,BL~,LFE~,C~,R~,L~
			0x06,							/* bmChannelConfigL2 */	//TBL,TFR,TFC,TFL,TC,SR~,SL~,BC
			0x00,							/* bmChannelConfigL3 */	//TSR,TSL,RLFE,LLFE,TFRC,TFLC,TBR,TBC
			0x00,							/* bmChannelConfigL4 */	//Raw,0,0,0,0,BRC,BLC,BC
			0x00							/* iChannelNames */
		},
		{/*
		  * Audio USB_OUT Format
	...
	{/*
	  * STD AS ISO IN Endpoint		//ZYNQ -> PC
	  */
		sizeof(USB_EP_DESC),		/* bLength */
		XUSBPS_TYPE_ENDPOINT_CFG_DESC,	/* bType */
...
#else
		0x01 | USB_ENDPOINT_SYNC_ADAPTIVE,	/* Isochronous, adaptive */
#endif
		0x00,				/* bMaxPacketSizeL */
		0x04,				/* bMaxPacketSizeH */
		AUDIO_INTERVAL			/* bInterval */
	...
	,
	{/*
	  * STD AS ISO Feedback IN Endpoint
	  */
		sizeof(UAC2_ISO_FB_EP_DESC),		/* bLength */
		XUSBPS_TYPE_ENDPOINT_CFG_DESC,	/* bType */
		0x01,			/* bEndpoint IN endpoint address 0 */
		0x11,			/* bmAttributes */
		0x00,				/* bMaxPacketSizeL */
		0x04,				/* bMaxPacketSizeH */
		AUDIO_INTERVAL			/* bInterval */
...
u32 XUsbPs_Ch9SetupStrDescReply(u8 *BufPtr, u32 BufLen, u8 Index)
{
	 ...
	 */
	if (0 == Index) {		//スタンダードストリングディスクリプタゼロ
...
//		XUsbPs_EpDisable((XUsbPs *)InstancePtr, ISO_EP2,
//						XUSBPS_EP_DIRECTION_OUT);
//
//		XUsbPs_EpDisable((XUsbPs *)InstancePtr, ISO_EP2,
//						XUSBPS_EP_DIRECTION_IN);
		...
		XUsbPs_SetupData *SetupData)
{
	...
	if ((SetupData->wIndex & 0xff) == 1) {			//インターフェース番号1(ZYNQ<-PC)
		if ((SetupData->wValue & 0xff) == 1) {		//オルタネイトセッティングでエンドポイントを有効化
			...
			RetVal = EpEnable((XUsbPs *)InstancePtr, ISO_EP,
					XUSBPS_EP_DIRECTION_IN,
					16, USB_EP_TYPE_ISOCHRONOUS);
			if (RetVal != XST_SUCCESS) {
				xil_printf("failed to enable ISOC FB OUT Ep\r\n");
				return;
			}
			XUsbPs_StreamOn(InstancePtr, ISO_EP,
					XUSBPS_EP_DIRECTION_IN,
	...
	if ((SetupData->wIndex & 0xff) == 2) {			//インターフェース番号2(ZYNQ->PC)
		if ((SetupData->wValue & 0xff) == 1) {
			XUsbPs_SetEpInterval(InstancePtr, ISO_EP2,
			...
			/* Endpoint enables - not needed for Control EP */
			/*------データストリームエンドポイント------*/
			RetVal = EpEnable((XUsbPs *)InstancePtr, ISO_EP2,
					XUSBPS_EP_DIRECTION_IN,
					MaxPktSize, USB_EP_TYPE_ISOCHRONOUS);
			if (RetVal != XST_SUCCESS) {
				xil_printf("failed to enable ISOC IN Ep\r\n");
			...
			XUsbPs_StreamOn(InstancePtr, ISO_EP2,
					XUSBPS_EP_DIRECTION_IN,
					BufferPtrTemp);
			/*------フィードバックエンドポイント------*/
//			RetVal = EpEnable((XUsbPs *)InstancePtr, ISO_EP2,
...
//					16, USB_EP_TYPE_ISOCHRONOUS);
//			if (RetVal != XST_SUCCESS) {
//				xil_printf("failed to enable ISOC FB IN Ep\r\n");
//				return;
//			}
//			XUsbPs_StreamOn(InstancePtr, ISO_EP2,
//					XUSBPS_EP_DIRECTION_OUT,
//					BufferPtrTemp);
			...
			/*------データストリームエンドポイント------*/
			XUsbPs_StreamOff(InstancePtr, ISO_EP2,
					XUSBPS_EP_DIRECTION_IN);
			/* Endpoint disables - not needed for Control EP */
			XUsbPs_EpDisable((XUsbPs *)InstancePtr, ISO_EP2,
					XUSBPS_EP_DIRECTION_IN);
